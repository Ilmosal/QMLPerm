...
from model_utils import chunk_vmapped_fn

jax.config.update("jax_enable_x64", True)

class AltKernelClassifier(BaseEstimator, ClassifierMixin):
    def __init__(
        self,
        ...
        r"""
        Kernel modified from the IQP kernel to test out different Feature maps
    ...
    def construct_circuit(self):
        ...
        def circuit(x):
            ...
            """
            qml.AngleEmbedding(x[: self.n_qubits_], wires=range(self.n_qubits_))
            qml.adjoint(qml.AngleEmbedding(x[self.n_qubits_ :], wires=range(self.n_qubits_)))
    ...
    def fit(self, X, y):
        ...
        self.svm.random_state = np.random.randint(0, 1000000)
